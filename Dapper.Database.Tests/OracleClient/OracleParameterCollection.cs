...
public class OracleParameterCollection : DbParameterCollection
{
    ...
    private void AssertValid()
    {
        ...
        for (var i = 0; i < Count; i++)
            Debug.Assert(ReferenceEquals(_parameters[i].RealParameter, _realParameters[i]),
                $"parameter {i} does not match");
    }
    ...
    public override void RemoveAt(string parameterName) => RemoveAtInternal(IndexOf(parameterName));
