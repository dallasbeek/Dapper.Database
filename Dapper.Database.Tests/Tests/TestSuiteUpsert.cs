...
public abstract partial class TestSuite
{
    ...
    public void UpsertPartial()
    {
        ...
        p.FirstName = "Greg";
        p.LastName = "Smith";
        Assert.True(db.Upsert(p, new[] { "LastName" }));
    ...
    public void UpsertPartialCallbacks()
    {
        ...
        Assert.True(db.Upsert(p, i => i.CreatedOn = now, u => u.UpdatedOn = now));
        Assert.True(p.IdentityId > 0);

        p.FirstName = "Greg";
        p.LastName = "Smith";
        p.CreatedOn = DateTime.UtcNow;
        Assert.True(db.Upsert(p, new[] { "LastName", "CreatedOn", "UpdatedOn" }, i => i.CreatedOn = now,
            u => u.UpdatedOn = now));
    ...
    [Fact]
    [Trait("Category", "Upsert")]
    public void UpsertConcurrencyCheckNotModified()
    {
        ...
        p.FirstName = "Greg";
        p.LastName = "Smith";
        Assert.True(db.Upsert(p), "Concurrent fields unchanged");
