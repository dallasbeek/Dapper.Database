...
public abstract partial class TestSuite
{
    ...
    public void CountWithSelectClauseParameter()
    {
        using var db = GetSqlDatabase();
        Assert.Equal(89,
            db.Count<Product>($"select * from Product where Color = {P}Color", new { Color = "Black" }));
    }
    ...
    public void CountShortCircuit()
    {
        using var db = GetSqlDatabase();
        Assert.Equal(89,
            db.Count<Product>($";select count(*) from Product where Color = {P}Color", new { Color = "Black" }));
    }
}
