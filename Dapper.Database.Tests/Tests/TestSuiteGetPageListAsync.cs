...
public abstract partial class TestSuite
{
    ...
    {
        using var db = GetSqlDatabase();
        var lst = await db.GetPageListAsync<Product>(5, 10, $"where Color = {P}Color order by lower(Name)",
            new { Color = "Black" });
        Assert.Equal(10, lst.Count());
        var item = lst.Single(p => p.ProductID == 816);
        ValidateProduct816(item);
    ...
    {
        using var db = GetSqlDatabase();
        var lst = await db.GetPageListAsync<Product>(4, 10,
    ...
    {
        using var db = GetSqlDatabase();
        var lst = await db.GetPageListAsync<Product>(5, 10,
