    ...
    public abstract partial class TestSuite
    {
        ...
        public void GetFirstPartialBySelect()
        {
            using ( var db = GetSqlDatabase() )
            {
                ...
                var item = db.GetFirst<Product>($"select p.ProductId, p.rowguid AS GuidId, {nameColumn} from Product p where p.Color = {P}Color and p.ProductId >= {P}ProductId order by p.ProductId", new { Color = "Black", ProductId = 816 });
